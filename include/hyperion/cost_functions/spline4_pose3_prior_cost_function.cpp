...
auto Spline4Pose3PriorCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 6, 1>>{residuals} = Spline4Pose3DeltaFactor<Scalar>(
        lambdas_, _x0, _x1, _x2, _x3, _x4, y_, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 6, 1>>{residuals} =
      Spline4Pose3DeltaFactorWithJacobians12345<Scalar>(
          lambdas_, _x0, _x1, _x2, _x3, _x4, y_, sqrt_info_, sym::kDefaultEpsilon<Scalar>,
