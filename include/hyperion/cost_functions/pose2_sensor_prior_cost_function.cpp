...
auto Pose2SensorPriorCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
                                            Scalar** jacobians) const -> bool {
  // Create aliases.
  const auto _x = sym::StorageOps<sym::Pose2<Scalar>>::FromStorage(parameters[0]);
  const auto _x_T_y = sym::StorageOps<sym::Pose2<Scalar>>::FromStorage(parameters[1]);

  // Evaluation without Jacobians.
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 3, 1>>{residuals} =
        Pose2SensorDeltaFactor<Scalar>(_x, _x_T_y, y_, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 3, 1>>{residuals} =
      Pose2SensorDeltaFactorWithJacobians01<Scalar>(
