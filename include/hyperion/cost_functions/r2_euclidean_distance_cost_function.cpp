...
#include "r2_euclidean_distance_cost_function.hpp"

#include <sym/ops/storage_ops.h>
#include <sym/util/epsilon.h>

#include "sym/r2_euclidean_distance_factor.hpp"
#include "sym/r2_euclidean_distance_factor_with_jacobians02.hpp"

namespace hyperion {

using namespace sym_hyperion;

auto R2EuclideanDistanceCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} =
        R2EuclideanDistanceFactor<Scalar>(_x, x_d_y_, _y, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} =
      R2EuclideanDistanceFactorWithJacobians02<Scalar>(
