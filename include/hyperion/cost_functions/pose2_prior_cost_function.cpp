...
auto Pose2PriorCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 3, 1>>{residuals} =
        Pose2DeltaFactor<Scalar>(_x, y_, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 3, 1>>{residuals} = Pose2DeltaFactorWithJacobian0<Scalar>(
