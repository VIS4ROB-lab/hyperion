...
auto Rot2BetweenCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} =
        Rot2BetweenFactor<Scalar>(_x, x_T_y_, _y, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} = Rot2BetweenFactorWithJacobians02<Scalar>(
