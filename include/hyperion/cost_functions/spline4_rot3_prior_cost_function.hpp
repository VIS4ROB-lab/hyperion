...
class Spline4Rot3PriorCostFunction final
 ...
 public:
  /// Constructor.
  explicit Spline4Rot3PriorCostFunction(const Eigen::Matrix<Scalar, 4, 1>& lambdas,
                                        const sym::Rot3<Scalar>& prior,
                                        const Eigen::Matrix<Scalar, 3, 3>& sqrtInfo)
      : lambdas_{lambdas}, y_{prior}, sqrt_info_{sqrtInfo} {}

  /// lambdas accessor.
  [[nodiscard]] auto lambdas() const -> const Eigen::Matrix<Scalar, 4, 1>& {
    return lambdas_;
  }

  /// lambdas modifier.
  auto lambdas() -> Eigen::Matrix<Scalar, 4, 1>& {
    return lambdas_;
  }

  /// prior accessor.
  [[nodiscard]] auto prior() const -> const sym::Rot3<Scalar>& {
    return y_;
  }

  /// prior modifier.
  auto prior() -> sym::Rot3<Scalar>& {
    return y_;
  }
 ...
 private:
  Eigen::Matrix<Scalar, 4, 1> lambdas_;
  sym::Rot3<Scalar> y_;
