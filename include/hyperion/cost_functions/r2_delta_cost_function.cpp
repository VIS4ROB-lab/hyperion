...
#include "r2_delta_cost_function.hpp"

#include <sym/ops/storage_ops.h>
#include <sym/util/epsilon.h>

#include "sym/r2_delta_factor.hpp"
#include "sym/r2_delta_factor_with_jacobians01.hpp"

namespace hyperion {

using namespace sym_hyperion;

auto R2DeltaCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 2, 1>>{residuals} =
        R2DeltaFactor<Scalar>(_x, _y, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 2, 1>>{residuals} = R2DeltaFactorWithJacobians01<Scalar>(
