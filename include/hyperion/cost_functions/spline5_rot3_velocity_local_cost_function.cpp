...
auto Spline5Rot3VelocityLocalCostFunction::evaluate(Scalar const* const* parameters,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 3, 1>>{residuals} =
        Spline5Rot3VelocityLocalFactor<Scalar>(dt_, lambdas_, _x0, _x1, _x2, _x3, _x4, _x5,
                                               velocity_, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 3, 1>>{residuals} =
      Spline5Rot3VelocityLocalFactorWithJacobians234567<Scalar>(
