...
class Rot3BetweenCostFunction final
    : public SizedCostFunction<3, ConstexprGroupDims<4, 3>, ConstexprGroupDims<4, 3>> {
 public:
  /// Constructor.
  explicit Rot3BetweenCostFunction(const sym::Rot3<Scalar>& xTy,
                                   const Eigen::Matrix<Scalar, 3, 3>& sqrtInfo)
      : x_T_y_{xTy}, sqrt_info_{sqrtInfo} {}

  /// xTy accessor.
  [[nodiscard]] auto xTy() const -> const sym::Rot3<Scalar>& {
    return x_T_y_;
  }

  /// xTy modifier.
  auto xTy() -> sym::Rot3<Scalar>& {
    return x_T_y_;
  }
