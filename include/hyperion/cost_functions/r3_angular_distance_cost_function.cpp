...
auto R3AngularDistanceCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} =
        R3AngularDistanceFactor<Scalar>(_x, x_d_y_, _y, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} =
      R3AngularDistanceFactorWithJacobians02<Scalar>(
