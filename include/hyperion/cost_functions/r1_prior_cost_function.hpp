...
class R1PriorCostFunction final : public SizedCostFunction<1, ConstexprGroupDims<1, 1>> {
 public:
  /// Constructor.
  explicit R1PriorCostFunction(const Eigen::Matrix<Scalar, 1, 1>& prior,
                               const Eigen::Matrix<Scalar, 1, 1>& sqrtInfo)
      : y_{prior}, sqrt_info_{sqrtInfo} {}

  /// prior accessor.
  [[nodiscard]] auto prior() const -> const Eigen::Matrix<Scalar, 1, 1>& {
    return y_;
  }

  /// prior modifier.
  auto prior() -> Eigen::Matrix<Scalar, 1, 1>& {
    return y_;
  }
