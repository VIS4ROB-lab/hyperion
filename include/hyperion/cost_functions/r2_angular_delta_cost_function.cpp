...
auto R2AngularDeltaCostFunction::evaluate(Scalar const* const* parameters, Scalar* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} =
        R2AngularDeltaFactor<Scalar>(_x, _y, sqrt_info_, sym::kDefaultEpsilon<Scalar>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<Scalar, 1, 1>>{residuals} = R2AngularDeltaFactorWithJacobians01<Scalar>(
