...
class Spline5Pose3VelocityGlobalCostFunction final
    : public SizedCostFunction<6, ConstexprGroupDims<7, 6>, ConstexprGroupDims<7, 6>,
                               ConstexprGroupDims<7, 6>, ConstexprGroupDims<7, 6>,
                               ConstexprGroupDims<7, 6>, ConstexprGroupDims<7, 6>> {
 public:
  /// Constructor.
  explicit Spline5Pose3VelocityGlobalCostFunction(const Scalar dt,
                                                  const Eigen::Matrix<Scalar, 5, 2>& lambdas,
  ...
  /// lambdas accessor.
  [[nodiscard]] auto lambdas() const -> const Eigen::Matrix<Scalar, 5, 2>& {
    return lambdas_;
  }

  /// lambdas modifier.
  auto lambdas() -> Eigen::Matrix<Scalar, 5, 2>& {
    return lambdas_;
  }

  /// velocity accessor.
  [[nodiscard]] auto velocity() const -> const Eigen::Matrix<Scalar, 6, 1>& {
    return velocity_;
  }

  /// velocity modifier.
  auto velocity() -> Eigen::Matrix<Scalar, 6, 1>& {
    return velocity_;
  }
 ...
 private:
  Scalar dt_;
  Eigen::Matrix<Scalar, 5, 2> lambdas_;
  Eigen::Matrix<Scalar, 6, 1> velocity_;
