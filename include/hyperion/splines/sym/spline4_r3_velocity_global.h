 ...
 *
 * Symbolic function: spline4_r3_velocity
 *
 * Args:
 *     dt: Scalar
 *     lambdas: Matrix42
...
Eigen::Matrix<Scalar, 3, 1> Spline4R3VelocityGlobal(
    const Scalar dt, const Eigen::Matrix<Scalar, 4, 2>& lambdas,
    ...
    const Eigen::Matrix<Scalar, 3, 1>& x4, const Scalar epsilon) {
  // Total ops: 37
  ...
  _res(2, 0) =
      _tmp0 * (lambdas(0, 1) * (-x0(2, 0) + x1(2, 0)) + lambdas(1, 1) * (-x1(2, 0) + x2(2, 0)) +
               lambdas(2, 1) * (-x2(2, 0) + x3(2, 0)) + lambdas(3, 1) * (-x3(2, 0) + x4(2, 0)));
