...
Eigen::Matrix<Scalar, 4, 5> CumulativeUniformBSplineLambdas54(const Scalar ut) {
  // Total ops: 43
  ...
  const Scalar _tmp9 = (Scalar(1) / Scalar(2)) * _tmp0;
  const Scalar _tmp10 = _tmp9 + Scalar(1) / Scalar(6);
  const Scalar _tmp11 = (Scalar(1) / Scalar(2)) * _tmp4;
  const Scalar _tmp12 = (Scalar(3) / Scalar(2)) * _tmp0;
  const Scalar _tmp13 = 3 * ut;
  ...
  _res(3, 0) = _tmp3;
  _res(0, 1) = _tmp10 - _tmp5 - _tmp8;
  _res(1, 1) = -_tmp0 + _tmp11 + Scalar(2) / Scalar(3);
  _res(2, 1) = _tmp10 - _tmp11 + _tmp8;
  _res(3, 1) = _tmp5;
  _res(0, 2) = -_tmp9 + ut + Scalar(-1) / Scalar(2);
  _res(1, 2) = _tmp12 - 2 * ut;
  _res(2, 2) = -_tmp12 + ut + Scalar(1) / Scalar(2);
  _res(3, 2) = _tmp9;
  _res(0, 3) = 1 - ut;
  _res(1, 3) = _tmp13 - 2;
  _res(2, 3) = 1 - _tmp13;
