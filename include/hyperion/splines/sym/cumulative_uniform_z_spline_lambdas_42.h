...
Eigen::Matrix<Scalar, 3, 3> CumulativeUniformZSplineLambdas42(const Scalar ut) {
  // Total ops: 25
  ...
  const Scalar _tmp4 = (Scalar(3) / Scalar(2)) * _tmp1;
  const Scalar _tmp5 = 3 * ut;

  // Output terms (1)
  Eigen::Matrix<Scalar, 3, 3> _res;

  _res(0, 0) = _tmp0 - _tmp1 + _tmp3 + 1;
  _res(1, 0) = _tmp0 - _tmp2 + _tmp4;
  _res(2, 0) = -Scalar(1) / Scalar(2) * _tmp1 + _tmp3;
  _res(0, 1) = _tmp4 - 2 * ut + Scalar(1) / Scalar(2);
  _res(1, 1) = -3 * _tmp1 + _tmp5 + Scalar(1) / Scalar(2);
  _res(2, 1) = _tmp4 - ut;
  _res(0, 2) = _tmp5 - 2;
  _res(1, 2) = 3 - 6 * ut;
  _res(2, 2) = _tmp5 - 1;
