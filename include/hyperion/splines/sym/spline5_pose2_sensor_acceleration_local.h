...
Eigen::Matrix<Scalar, 3, 1> Spline5Pose2SensorAccelerationLocal(
  ...
  const Scalar _tmp11 =
      _tmp0 * (_tmp10 * lambdas(0, 2) + _tmp2 * lambdas(2, 2) + _tmp4 * lambdas(3, 2) +
  ...
  const Scalar _tmp34 = std::cos(_tmp31);
  const Scalar _tmp35 = -_tmp30 * _tmp32 + _tmp33 * _tmp34;
  const Scalar _tmp36 = -_x2[2] + _x3[2];
  const Scalar _tmp37 = -_x1[2] + _x2[2];
  const Scalar _tmp38 = -_x4[2] + _x5[2];
  const Scalar _tmp39 = -_x0[2] + _x1[2];
  const Scalar _tmp40 = -_x3[2] + _x4[2];
  const Scalar _tmp41 = _tmp36 * lambdas(2, 1) + _tmp37 * lambdas(1, 1) + _tmp38 * lambdas(4, 1) +
                        _tmp39 * lambdas(0, 1) + _tmp40 * lambdas(3, 1);
  const Scalar _tmp42 = _tmp30 * _tmp34 + _tmp32 * _tmp33;
  const Scalar _tmp43 = -_x2[3] + _x3[3];
  const Scalar _tmp44 = -_x1[3] + _x2[3];
  const Scalar _tmp45 = -_x4[3] + _x5[3];
  const Scalar _tmp46 = -_x0[3] + _x1[3];
  const Scalar _tmp47 = -_x3[3] + _x4[3];
  const Scalar _tmp48 = _tmp43 * lambdas(2, 1) + _tmp44 * lambdas(1, 1) + _tmp45 * lambdas(4, 1) +
                        _tmp46 * lambdas(0, 1) + _tmp47 * lambdas(3, 1);
  const Scalar _tmp49 = _tmp10 * lambdas(0, 1) + _tmp2 * lambdas(2, 1) + _tmp4 * lambdas(3, 1) +
                        _tmp6 * lambdas(4, 1) + _tmp8 * lambdas(1, 1);
  const Scalar _tmp50 = _tmp36 * lambdas(2, 2) + _tmp37 * lambdas(1, 2) + _tmp38 * lambdas(4, 2) +
                        _tmp39 * lambdas(0, 2) + _tmp40 * lambdas(3, 2);
  const Scalar _tmp51 = _tmp43 * lambdas(2, 2) + _tmp44 * lambdas(1, 2) + _tmp45 * lambdas(4, 2) +
                        _tmp46 * lambdas(0, 2) + _tmp47 * lambdas(3, 2);
  const Scalar _tmp52 =
      _tmp0 * (_tmp35 * _tmp51 - _tmp42 * _tmp50 - _tmp49 * (_tmp35 * _tmp41 + _tmp42 * _tmp48));
  const Scalar _tmp53 =
      _tmp0 * (_tmp35 * _tmp50 + _tmp42 * _tmp51 + _tmp49 * (_tmp35 * _tmp48 - _tmp41 * _tmp42));

  // Output terms (1)
  Eigen::Matrix<Scalar, 3, 1> _res;

  _res(0, 0) = _tmp11;
  _res(1, 0) = -_tmp11 * (_x_T_s[0] * _x_T_s[3] - _x_T_s[1] * _x_T_s[2]) + _tmp52 * _x_T_s[1] +
               _tmp53 * _x_T_s[0];
  _res(2, 0) = _tmp11 * (_x_T_s[0] * _x_T_s[2] + _x_T_s[1] * _x_T_s[3]) + _tmp52 * _x_T_s[0] -
               _tmp53 * _x_T_s[1];
