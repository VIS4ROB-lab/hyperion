...
auto R3EuclideanDistanceCostFunction::Evaluate(double const* const* parameters, double* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<double, 1, 1>>{residuals} =
        R3EuclideanDistanceFactor<double>(_x, x_d_y_, _y, sqrt_info_, sym::kDefaultEpsilon<double>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<double, 1, 1>>{residuals} =
      R3EuclideanDistanceFactorWithJacobians02<double>(
