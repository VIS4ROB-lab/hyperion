...
class Spline4Pose3AccelerationGlobalCostFunction final
    : public ::ceres::SizedCostFunction<6, 7, 7, 7, 7, 7> {
 public:
  /// Constructor.
  explicit Spline4Pose3AccelerationGlobalCostFunction(
      const double dt, const Eigen::Matrix<double, 4, 3>& lambdas,
  ...
  /// lambdas accessor.
  [[nodiscard]] auto lambdas() const -> const Eigen::Matrix<double, 4, 3>& {
    return lambdas_;
  }

  /// lambdas modifier.
  auto lambdas() -> Eigen::Matrix<double, 4, 3>& {
    return lambdas_;
  }
 ...
 private:
  double dt_;
  Eigen::Matrix<double, 4, 3> lambdas_;
