...
class Spline5Pose3VelocityGlobalCostFunction final
 ...
 public:
  /// Constructor.
  explicit Spline5Pose3VelocityGlobalCostFunction(const double dt,
                                                  const Eigen::Matrix<double, 5, 2>& lambdas,
  ...
  /// lambdas accessor.
  [[nodiscard]] auto lambdas() const -> const Eigen::Matrix<double, 5, 2>& {
    return lambdas_;
  }

  /// lambdas modifier.
  auto lambdas() -> Eigen::Matrix<double, 5, 2>& {
    return lambdas_;
  }
 ...
 private:
  double dt_;
  Eigen::Matrix<double, 5, 2> lambdas_;
