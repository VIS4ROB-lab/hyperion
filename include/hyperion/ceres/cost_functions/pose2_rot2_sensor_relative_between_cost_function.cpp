...
auto Pose2Rot2SensorRelativeBetweenCostFunction::Evaluate(double const* const* parameters,
                                                          double* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<double, 1, 1>>{residuals} =
        Pose2Rot2SensorRelativeBetweenFactor<double>(_x, _x_T_a, a_T_b_, _y, _y_T_b, sqrt_info_,
  ...
  Eigen::Map<Eigen::Matrix<double, 1, 1>>{residuals} =
      Pose2Rot2SensorRelativeBetweenFactorWithJacobians0134<double>(
