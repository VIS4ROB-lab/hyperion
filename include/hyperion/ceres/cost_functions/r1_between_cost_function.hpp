...
class R1BetweenCostFunction final : public ::ceres::SizedCostFunction<1, 1, 1> {
 public:
  /// Constructor.
  explicit R1BetweenCostFunction(const Eigen::Matrix<double, 1, 1>& xTy,
                                 const Eigen::Matrix<double, 1, 1>& sqrtInfo)
      : x_T_y_{xTy}, sqrt_info_{sqrtInfo} {}

  /// xTy accessor.
  [[nodiscard]] auto xTy() const -> const Eigen::Matrix<double, 1, 1>& {
    return x_T_y_;
  }

  /// xTy modifier.
  auto xTy() -> Eigen::Matrix<double, 1, 1>& {
    return x_T_y_;
  }
 ...
 private:
  Eigen::Matrix<double, 1, 1> x_T_y_;
