...
auto Pose3BetweenCostFunction::Evaluate(double const* const* parameters, double* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<double, 6, 1>>{residuals} =
        Pose3BetweenFactor<double>(_x, x_T_y_, _y, sqrt_info_, sym::kDefaultEpsilon<double>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<double, 6, 1>>{residuals} = Pose3BetweenFactorWithJacobians02<double>(
