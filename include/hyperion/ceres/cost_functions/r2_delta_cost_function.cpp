...
auto R2DeltaCostFunction::Evaluate(double const* const* parameters, double* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<double, 2, 1>>{residuals} =
        R2DeltaFactor<double>(_x, _y, sqrt_info_, sym::kDefaultEpsilon<double>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<double, 2, 1>>{residuals} = R2DeltaFactorWithJacobians01<double>(
