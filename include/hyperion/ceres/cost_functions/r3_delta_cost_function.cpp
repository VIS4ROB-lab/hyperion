...
auto R3DeltaCostFunction::Evaluate(double const* const* parameters, double* residuals,
  ...
  if (!jacobians) {
    Eigen::Map<Eigen::Matrix<double, 3, 1>>{residuals} =
        R3DeltaFactor<double>(_x, _y, sqrt_info_, sym::kDefaultEpsilon<double>);
    return true;
  }

  // Evaluation with Jacobians.
  Eigen::Map<Eigen::Matrix<double, 3, 1>>{residuals} = R3DeltaFactorWithJacobians01<double>(
