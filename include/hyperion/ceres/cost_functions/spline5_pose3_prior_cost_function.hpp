...
class Spline5Pose3PriorCostFunction final : public ::ceres::SizedCostFunction<6, 7, 7, 7, 7, 7, 7> {
 public:
  /// Constructor.
  explicit Spline5Pose3PriorCostFunction(const Eigen::Matrix<double, 5, 1>& lambdas,
                                         const sym::Pose3<double>& prior,
                                         const Eigen::Matrix<double, 6, 6>& sqrtInfo)
      : lambdas_{lambdas}, y_{prior}, sqrt_info_{sqrtInfo} {}

  /// lambdas accessor.
  [[nodiscard]] auto lambdas() const -> const Eigen::Matrix<double, 5, 1>& {
    return lambdas_;
  }

  /// lambdas modifier.
  auto lambdas() -> Eigen::Matrix<double, 5, 1>& {
    return lambdas_;
  }
 ...
 private:
  Eigen::Matrix<double, 5, 1> lambdas_;
  sym::Pose3<double> y_;
