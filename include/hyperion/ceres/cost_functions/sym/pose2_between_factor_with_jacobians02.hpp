...
Eigen::Matrix<Scalar, 3, 1> Pose2BetweenFactorWithJacobians02(
  ...
  const Scalar _tmp26 = _tmp24 / (_tmp24 + std::pow(_tmp8, Scalar(2)));
  const Scalar _tmp27 =
      _tmp26 * (_tmp23 * (-_tmp10 + _tmp22 * _x_T_y[0]) - _tmp25 * (_tmp22 * _x_T_y[1] + _tmp7));
  const Scalar _tmp28 = _tmp14 - _tmp15 - _tmp16;
  const Scalar _tmp29 = -_tmp18 + _tmp19 + _tmp20;
  const Scalar _tmp30 =
      _tmp27 * sqrt_info(0, 0) + _tmp28 * sqrt_info(0, 2) + _tmp29 * sqrt_info(0, 1);
  const Scalar _tmp31 =
      _tmp27 * sqrt_info(1, 0) + _tmp28 * sqrt_info(1, 2) + _tmp29 * sqrt_info(1, 1);
  const Scalar _tmp32 =
      _tmp27 * sqrt_info(2, 0) + _tmp28 * sqrt_info(2, 2) + _tmp29 * sqrt_info(2, 1);
  const Scalar _tmp33 = _x[0] * sqrt_info(0, 1);
  const Scalar _tmp34 = _x[1] * sqrt_info(0, 2);
  const Scalar _tmp35 = _x[0] * sqrt_info(1, 1);
  const Scalar _tmp36 = _x[1] * sqrt_info(1, 2);
  const Scalar _tmp37 = _x[0] * sqrt_info(2, 1);
  const Scalar _tmp38 = _x[1] * sqrt_info(2, 2);
  const Scalar _tmp39 = _x[0] * sqrt_info(0, 2);
  const Scalar _tmp40 = _x[1] * sqrt_info(0, 1);
  const Scalar _tmp41 = _x[0] * sqrt_info(1, 2);
  const Scalar _tmp42 = _x[1] * sqrt_info(1, 1);
  const Scalar _tmp43 = _x[0] * sqrt_info(2, 2);
  const Scalar _tmp44 = _x[1] * sqrt_info(2, 1);
  const Scalar _tmp45 = -_tmp4 + _tmp5;
  const Scalar _tmp46 =
      _tmp26 * (_tmp23 * (-_tmp45 * _x_T_y[1] + _tmp9) - _tmp25 * (_tmp3 + _tmp45 * _x_T_y[0]));
  const Scalar _tmp47 = _tmp46 * _y[1];
  const Scalar _tmp48 = _tmp46 * _y[0];
  ...
  if (res_D_x != nullptr) {
    ...
    _res_D_x(0, 0) = -_tmp30 * _x[1];
    _res_D_x(1, 0) = -_tmp31 * _x[1];
    _res_D_x(2, 0) = -_tmp32 * _x[1];
    _res_D_x(0, 1) = _tmp30 * _x[0];
    _res_D_x(1, 1) = _tmp31 * _x[0];
    _res_D_x(2, 1) = _tmp32 * _x[0];
    _res_D_x(0, 2) = -_tmp33 + _tmp34;
    _res_D_x(1, 2) = -_tmp35 + _tmp36;
    _res_D_x(2, 2) = -_tmp37 + _tmp38;
    _res_D_x(0, 3) = -_tmp39 - _tmp40;
    _res_D_x(1, 3) = -_tmp41 - _tmp42;
    _res_D_x(2, 3) = -_tmp43 - _tmp44;
  }

  if (res_D_y != nullptr) {
    Eigen::Map<Eigen::Matrix<Scalar, 3, 4, Eigen::RowMajor>> _res_D_y{res_D_y};

    _res_D_y(0, 0) = -_tmp47 * sqrt_info(0, 0);
    _res_D_y(1, 0) = -_tmp47 * sqrt_info(1, 0);
    _res_D_y(2, 0) = -_tmp47 * sqrt_info(2, 0);
    _res_D_y(0, 1) = _tmp48 * sqrt_info(0, 0);
    _res_D_y(1, 1) = _tmp48 * sqrt_info(1, 0);
    _res_D_y(2, 1) = _tmp48 * sqrt_info(2, 0);
    _res_D_y(0, 2) = _tmp33 - _tmp34;
    _res_D_y(1, 2) = _tmp35 - _tmp36;
    _res_D_y(2, 2) = _tmp37 - _tmp38;
    _res_D_y(0, 3) = _tmp39 + _tmp40;
    _res_D_y(1, 3) = _tmp41 + _tmp42;
    _res_D_y(2, 3) = _tmp43 + _tmp44;
  }
