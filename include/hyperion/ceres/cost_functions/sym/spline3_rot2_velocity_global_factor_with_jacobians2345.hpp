 ...
 *
 * Symbolic function: spline3_rot2_velocity_factor
 *
 * Args:
 *     dt: Scalar
 *     lambdas: Matrix32
...
Eigen::Matrix<Scalar, 1, 1> Spline3Rot2VelocityGlobalFactorWithJacobians2345(
    const Scalar dt, const Eigen::Matrix<Scalar, 3, 2>& lambdas, const sym::Rot2<Scalar>& x0,
    const sym::Rot2<Scalar>& x1, const sym::Rot2<Scalar>& x2, const sym::Rot2<Scalar>& x3,
    const Eigen::Matrix<Scalar, 1, 1>& velocity, const Eigen::Matrix<Scalar, 1, 1>& sqrt_info,
  ...
  const Scalar _tmp27 = _tmp23 * lambdas(0, 1) / (_tmp22 + _tmp23);
  const Scalar _tmp28 = _tmp26 * _tmp27 * (-_tmp22 * _tmp24 + _tmp25 * (-_tmp18 - _tmp19));
  const Scalar _tmp29 = Scalar(1.0) / (_tmp14);
  const Scalar _tmp30 = std::pow(_tmp10, Scalar(2));
  const Scalar _tmp31 = std::pow(_tmp14, Scalar(2));
  const Scalar _tmp32 = Scalar(1.0) / (_tmp31);
  const Scalar _tmp33 = _tmp31 * lambdas(1, 1) / (_tmp30 + _tmp31);
  const Scalar _tmp34 =
      _tmp26 * (_tmp27 * (-_tmp17 * _tmp24 * (_tmp15 - _tmp16) + _tmp20 * _tmp25) +
                _tmp33 * (_tmp29 * (-_tmp11 - _tmp12) - _tmp30 * _tmp32));
  const Scalar _tmp35 = Scalar(1.0) / (_tmp7);
  const Scalar _tmp36 = std::pow(_tmp3, Scalar(2));
  const Scalar _tmp37 = std::pow(_tmp7, Scalar(2));
  const Scalar _tmp38 = Scalar(1.0) / (_tmp37);
  const Scalar _tmp39 = _tmp37 * lambdas(2, 1) / (_tmp36 + _tmp37);
  const Scalar _tmp40 = _tmp26 * (_tmp33 * (-_tmp10 * _tmp32 * (-_tmp8 + _tmp9) + _tmp13 * _tmp29) +
                                  _tmp39 * (_tmp35 * (-_tmp4 - _tmp5) - _tmp36 * _tmp38));
  const Scalar _tmp41 = _tmp26 * _tmp39 * (-_tmp3 * _tmp38 * (_tmp1 - _tmp2) + _tmp35 * _tmp6);

  // Output terms (5)
  Eigen::Matrix<Scalar, 1, 1> _res;

  _res(0, 0) = sqrt_info(0, 0) * (_tmp0 * (lambdas(0, 1) * std::atan2(_tmp17, _tmp21) +
                                           lambdas(1, 1) * std::atan2(_tmp10, _tmp14) +
                                           lambdas(2, 1) * std::atan2(_tmp3, _tmp7)) -
                                  velocity(0, 0));

  if (res_D_x0 != nullptr) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 2>> _res_D_x0{res_D_x0};

    _res_D_x0(0, 0) = -_tmp28 * _x0[1];
    _res_D_x0(0, 1) = _tmp28 * _x0[0];
  }

  if (res_D_x1 != nullptr) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 2>> _res_D_x1{res_D_x1};

    _res_D_x1(0, 0) = -_tmp34 * _x1[1];
    _res_D_x1(0, 1) = _tmp34 * _x1[0];
  }

  if (res_D_x2 != nullptr) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 2>> _res_D_x2{res_D_x2};

    _res_D_x2(0, 0) = -_tmp40 * _x2[1];
    _res_D_x2(0, 1) = _tmp40 * _x2[0];
  }

  if (res_D_x3 != nullptr) {
    Eigen::Map<Eigen::Matrix<Scalar, 1, 2>> _res_D_x3{res_D_x3};

    _res_D_x3(0, 0) = -_tmp41 * _x3[1];
    _res_D_x3(0, 1) = _tmp41 * _x3[0];
  }
