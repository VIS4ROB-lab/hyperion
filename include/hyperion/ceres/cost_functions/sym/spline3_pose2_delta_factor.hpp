...
Eigen::Matrix<Scalar, 3, 1> Spline3Pose2DeltaFactor(
    ...
    const sym::Pose2<Scalar>& y, const Eigen::Matrix<Scalar, 3, 3>& sqrt_info,
  ...
  const Scalar _tmp2 = _x2[0] * _x3[0] + _x2[1] * _x3[1];
  const Scalar _tmp3 =
  ...
  const Scalar _tmp8 = std::cos(_tmp3);
  const Scalar _tmp9 = std::cos(_tmp6);
  const Scalar _tmp10 = -_tmp4 * _tmp7 + _tmp8 * _tmp9;
  const Scalar _tmp11 = _x0[0] * _x1[0] + _x0[1] * _x1[1];
  const Scalar _tmp12 =
      lambdas(0, 0) *
      std::atan2(_x0[0] * _x1[1] - _x0[1] * _x1[0],
                 _tmp11 + epsilon * ((((_tmp11) > 0) - ((_tmp11) < 0)) + Scalar(0.5)));
  const Scalar _tmp13 = std::cos(_tmp12);
  const Scalar _tmp14 = _tmp4 * _tmp9 + _tmp7 * _tmp8;
  const Scalar _tmp15 = std::sin(_tmp12);
  const Scalar _tmp16 = _tmp10 * _tmp13 - _tmp14 * _tmp15;
  const Scalar _tmp17 = _tmp10 * _tmp15 + _tmp13 * _tmp14;
  const Scalar _tmp18 = _tmp16 * _x0[1] + _tmp17 * _x0[0];
  const Scalar _tmp19 = _tmp16 * _x0[0] - _tmp17 * _x0[1];
  const Scalar _tmp20 = _tmp18 * _y[1] + _tmp19 * _y[0];
  const Scalar _tmp21 =
      std::atan2(_tmp18 * _y[0] - _tmp19 * _y[1],
                 _tmp20 + epsilon * ((((_tmp20) > 0) - ((_tmp20) < 0)) + Scalar(0.5)));

  // Output terms (1)
  Eigen::Matrix<Scalar, 3, 1> _res;

  _res(0, 0) = _tmp0 * sqrt_info(0, 1) + _tmp1 * sqrt_info(0, 2) + _tmp21 * sqrt_info(0, 0);
  _res(1, 0) = _tmp0 * sqrt_info(1, 1) + _tmp1 * sqrt_info(1, 2) + _tmp21 * sqrt_info(1, 0);
  _res(2, 0) = _tmp0 * sqrt_info(2, 1) + _tmp1 * sqrt_info(2, 2) + _tmp21 * sqrt_info(2, 0);
