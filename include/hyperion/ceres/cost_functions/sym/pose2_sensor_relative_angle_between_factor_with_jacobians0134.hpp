...
Eigen::Matrix<Scalar, 1, 1> Pose2SensorRelativeAngleBetweenFactorWithJacobians0134(
  ...
  const Scalar _tmp14 = std::atan2(_tmp10, _tmp13);
  const Scalar _tmp15 = std::sqrt(Scalar(std::pow(_tmp14, Scalar(2)) + epsilon));
  const Scalar _tmp16 = -_tmp1 - _tmp2;
  const Scalar _tmp17 = Scalar(1.0) / (_tmp13);
  const Scalar _tmp18 = std::pow(_tmp13, Scalar(2));
  const Scalar _tmp19 = _tmp10 / _tmp18;
  const Scalar _tmp20 =
      _tmp14 * _tmp18 * sqrt_info(0, 0) / (_tmp15 * (std::pow(_tmp10, Scalar(2)) + _tmp18));
  const Scalar _tmp21 =
      _tmp20 * (_tmp17 * (-_tmp11 + _tmp16 * _tmp7) - _tmp19 * (_tmp0 * _tmp16 + _tmp9));
  const Scalar _tmp22 = -_tmp5 - _tmp6;
  const Scalar _tmp23 =
      _tmp20 * (_tmp17 * (_tmp11 - _tmp22 * _tmp3) - _tmp19 * (_tmp22 * _tmp8 + _tmp4));

  // Output terms (5)
  Eigen::Matrix<Scalar, 1, 1> _res;

  _res(0, 0) = sqrt_info(0, 0) * (_tmp15 - a_d_b(0, 0));
  ...
  if (res_D_x_T_a != nullptr) {
    ...
    _res_D_x_T_a(0, 0) = -_tmp21 * _x_T_a[1];
    _res_D_x_T_a(0, 1) = _tmp21 * _x_T_a[0];
    _res_D_x_T_a(0, 2) = 0;
    _res_D_x_T_a(0, 3) = 0;
  }
  ...
  if (res_D_y_T_b != nullptr) {
    ...
    _res_D_y_T_b(0, 0) = -_tmp23 * _y_T_b[1];
    _res_D_y_T_b(0, 1) = _tmp23 * _y_T_b[0];
    _res_D_y_T_b(0, 2) = 0;
    _res_D_y_T_b(0, 3) = 0;
  }
