...
class R1PriorCostFunction final : public ::ceres::SizedCostFunction<1, 1> {
 public:
  /// Constructor.
  explicit R1PriorCostFunction(const Eigen::Matrix<double, 1, 1>& prior,
                               const Eigen::Matrix<double, 1, 1>& sqrtInfo)
      : y_{prior}, sqrt_info_{sqrtInfo} {}

  /// prior accessor.
  [[nodiscard]] auto prior() const -> const Eigen::Matrix<double, 1, 1>& {
    return y_;
  }

  /// prior modifier.
  auto prior() -> Eigen::Matrix<double, 1, 1>& {
    return y_;
  }
